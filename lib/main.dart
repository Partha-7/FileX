...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<String?> _wordToPdf() async {
    if (conversionServer.isEmpty) {
      throw Exception('Word conversion service is not configured yet.');
    }
    ...
    final response = await http.post(
      ...
      headers: {
        ...
        'X-File-Name': Uri.encodeComponent(selected.name),
      },
      body: await File(path).readAsBytes(),
    );

    if (response.statusCode != 200) {
      throw Exception('Conversion server returned ${response.statusCode}.');
    }
  ...
  Future<String?> _pdfToWord() async {
    if (conversionServer.isEmpty) {
      throw Exception('PDF conversion service is not configured yet.');
    }
      ...
      final scanAnother = await showDialog<bool>(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: Text(
              '${imagePaths.length} page(s) captured. Scan another page?',
            ),
            actions: [
              TextButton(
  ...
  Future<String?> _mergePdfs() async {
    ...
    if (paths.length < 2) {
      throw Exception('Unable to access the selected PDF files.');
    }
